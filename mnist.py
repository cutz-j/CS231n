...
mnist = input_data.read_data_sets('e:/data/MNIST_data', one_hot=True, reshape=False)
...
W1=tf.Variable(tf.truncated_normal(shape=[16*7*7,10]))
B1=tf.Variable(tf.truncated_normal(shape=[10]))
Pool2_flat=tf.reshape(Pool1,[-1,16*7*7])
OutputLayer=tf.matmul(Pool2_flat,W1)+B1


Loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=Y_Label, logits=OutputLayer))
